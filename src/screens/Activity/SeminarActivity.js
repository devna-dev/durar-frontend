            ...
            <Container style={styles.container}>
                <Content
                    ...
                    <Image
                        ...
                        source={{
                            uri: this.state.discussion.image
                                ? this.state.discussion.image
                                : 'https://alkbraquran.com/wp-content/uploads/2017/03/%D9%A2%D9%A0%D9%A1%D9%A6%D9%A0%D9%A9%D9%A0%D9%A6_%D9%A1%D9%A7%D9%A0%D9%A3%D9%A4%D9%A2.jpg',
                        }}
                    />
                    ...
                    <View style={styles.header1}>
                        <View style={[styles.item_view, {}]}>
                            <Text style={styles.active_item_text}>
                                {this.state.discussion.lecturer}
                            </Text>
                        </View>
                        <View style={[styles.item_view, {}]}>
                            <Text style={styles.active_item_text}>
                                المدة:{this.state.discussion.duration}
                            </Text>
                        </View>
                    </View>
                    <Text style={[styles.active_item_text1]}>
                        {this.state.discussion.description}
                    </Text>

                    <ImageBackground
                        source={require('../../assets/images/time_date_back.png')}
                        style={styles.header}>
                        <View style={{ width: '60%' }}>
                            <Text
                                style={[
                        ...
                        <View>
                            <Text
                                style={[styles.active_item_text, { height: 20, marginTop: 5 }]}>
                                الساعة
              </Text>
                            <Text
                                style={[
                                ...
                                {this.state.discussion.from_time}
                            </Text>
                        </View>
                    </ImageBackground>
                    {this.state.success && (
                        <Text
                            style={{
                                color: this.state.success ? colors.green1 : colors.error,
                                ...common.RegularFont,
                                fontSize: 16,
                                alignSelf: 'center',
                            }}>
                            {this.state.success
                                ? 'تم تسجيلك للمشاركة بالنشاط'
                                : 'أنت مسجل فعلياً في النشاط'}
                        </Text>
                    )}
                    {this.state.access ? (
                        Moment(this.state.discussion.date).format('DD-MMM-YYYY') ==
                            Moment(new Date()).format('DD-MMM-YYYY') ? (
                                ...
                                !this.state.discussion.registered && (
                                    <Button
                                        title={'تسجيل'}
                                        load={this.state.loading}
                                        onPress={async () => {
                                            if (this.props.route.params.dis) {
                                                this.setState({ loading: true });
                                                let result = await regiter_to_discussions(
                                                ...
                                                this.setState({ loading: true });
                                                let result = await regiter_to_activity(
                                                    this.props.route.params.id,
                                                );
                                                //console.log('registered', result)
                                                this.start();
